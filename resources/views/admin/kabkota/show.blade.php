...
@section('page-title', 'Detail Kabupaten/Kota')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ $kabkota->nama }}</h1>
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Detail kabupaten/kota</p>
                    </div>
                </div>
        ...
        <div class="p-6">
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Nama Kabupaten/Kota</dt>
                    <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $kabkota->nama }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Provinsi</dt>
                    <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $kabkota->provinsi->nama }}</dd>
                </div>                <div>
                ...
                <div>
                    ...
                    <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $kabkota->latitude ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Longitude</dt>
                    <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $kabkota->longitude ?? '-' }}</dd>
                </div>
                @if($kabkota->latitude && $kabkota->longitude)
                <div class="md:col-span-2">
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Koordinat</dt>
                    <dd class="mt-1">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200">
                            <i class="fas fa-map-marker-alt mr-1"></i>
                            {{ $kabkota->latitude }}, {{ $kabkota->longitude }}
                        </span>
                    </dd>
                </div>
                @endif<div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">ID Kabupaten/Kota</dt>
                    <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">#{{ $kabkota->id }}</dd>
                </div>
            </dl>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Faskes yang terdaftar di {{ $kabkota->nama }}</p>
        </div>

        @if($kabkota->faskes->count() > 0)
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($kabkota->faskes as $faskes)
        ...
        @else
            <div class="p-12 text-center">
                <div class="text-gray-500 dark:text-gray-400">
                    ...
                    <p class="mt-1">Belum ada faskes yang terdaftar di {{ $kabkota->nama }}</p>
                </div>
            </div>
        @endif
    </div>
