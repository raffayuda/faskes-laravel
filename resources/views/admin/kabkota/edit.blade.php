    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Edit Kabupaten/Kota</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Edit data kabupaten/kota</p>
                </div>
            </div>
        </div>

        <!-- Form -->
        <form action="{{ route('kabkota.update', $kabkota) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="provinsi_id" 
                    ...
                    @foreach($provinsis as $provinsi)
                        <option value="{{ $provinsi->id }}" {{ old('provinsi_id', $kabkota->provinsi_id) == $provinsi->id ? 'selected' : '' }}>
                            {{ $provinsi->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" 
                       id="latitude" 
                       name="latitude" 
                       value="{{ old('latitude', $kabkota->latitude) }}"
            ...
            <div>
                ...
                <input type="number" 
                       id="longitude" 
                       name="longitude" 
                       value="{{ old('longitude', $kabkota->longitude) }}"
