...
<div class="space-y-6">
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('faskes.update', $faskes) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="jenis_faskes_id" 
                                ...
                                @foreach($jenisFaskes as $jenis)
                                    <option value="{{ $jenis->id }}" {{ old('jenis_faskes_id', $faskes->jenis_faskes_id) == $jenis->id ? 'selected' : '' }}>
                                        {{ $jenis->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="kategori_id" 
                                ...
                                @foreach($kategori as $kat)
                                    <option value="{{ $kat->id }}" {{ old('kategori_id', $faskes->kategori_id) == $kat->id ? 'selected' : '' }}>
                                        {{ $kat->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="rating" 
                                ...
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ old('rating', $faskes->rating) == $i ? 'selected' : '' }}>
                                        {{ $i }} ⭐
                                    </option>
                                @endfor
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                @foreach($kabkota->groupBy('provinsi.nama') as $provinsiNama => $kabkotaList)
                                    <optgroup label="{{ $provinsiNama }}">
                                        @foreach($kabkotaList as $kab)
                                            <option value="{{ $kab->id }}" {{ old('kabkota_id', $faskes->kabkota_id) == $kab->id ? 'selected' : '' }}>
                                                {{ $kab->nama }}
                                            </option>
                                        @endforeach
                                    </optgroup>
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   id="latitude" 
                                   name="latitude" 
                                   value="{{ old('latitude', $faskes->latitude) }}"
                                   step="any"
                                   class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   id="longitude" 
                                   name="longitude" 
                                   value="{{ old('longitude', $faskes->longitude) }}"
                                   step="any"
                                   class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
                    ...
                    <div class="space-y-3 text-sm text-gray-600 dark:text-gray-400">
                        <div>
                            ...
                            <span class="text-gray-900 dark:text-white">{{ $faskes->created_at->format('d M Y H:i') }}</span>
                        </div>
                        
                        <div>
                            <span class="text-gray-500 dark:text-gray-400">Terakhir diupdate:</span>
                            <span class="text-gray-900 dark:text-white">{{ $faskes->updated_at->format('d M Y H:i') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
                    ...
                    <div class="space-y-2">
                        <a href="{{ route('faskes.show', $faskes) }}" 
                           class="w-full text-left px-3 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg flex items-center">
                            <i class="fas fa-eye mr-2"></i>
                            Lihat Detail
                        </a>

                        <button class="w-full text-left px-3 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg flex items-center">
